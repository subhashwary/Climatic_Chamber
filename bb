...
def start():
    global is_running, sampling_rate_sec, last_log_time
    global log_file, csv_writer, csv_filename
    ...
    csv_filename = f"sensor_log_{time.strftime('%Y%m%d_%H%M%S')}.csv"
    log_file = open(csv_filename, "w", newline="")
...
def stop():
    global is_running, frozen_values, log_file, csv_writer
...
def reset():
    global is_running, sample_counter, last_log_time
...
def read_serial():
    ...
    try:
        ...
        global last_log_time, sample_counter, new_sample_ready
        ...
        if is_running:
            ...
            if now - last_log_time >= sampling_rate_sec:
                ...
                row = [
                    ...
                    *last_values["pressure"],
                    *[(4*v)+5 for v in last_values["pressure"]],

                    *last_values["temperature"],
                    *[(8*v)+10 for v in last_values["temperature"]]
                ]

                # 🔥 WRITE TO CSV FILE (REAL-TIME LOGGING)
                if csv_writer:
                    csv_writer.writerow(row)
                    log_file.flush()

                # 🔥 ALSO STORE FOR /download_csv API
                with data_lock:
                    data_log.append({
